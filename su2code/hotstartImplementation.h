					 ...
					 const double delta)
{
	...
	for (x[0] = 0; x[0] < static_cast<long int>(timeSize); x[0]++)
		for (x[1] = 0; x[1] < static_cast<long int>(spaceSize); x[1]++)
			for (x[2] = 0; x[2] < static_cast<long int>(spaceSize); x[2]++)
				for (x[3] = 0; x[3] < static_cast<long int>(spaceSize); x[3]++)
